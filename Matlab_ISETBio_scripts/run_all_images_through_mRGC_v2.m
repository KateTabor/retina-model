function outFiles = run_all_images_through_mRGC_v2()
...
% Input: the generator's dated folder (matches pilot_patch.yaml naming)
rootDir = fullfile('/Users/kate/Documents/retina-model', ...
                   sprintf('%s_pilot_patch', char(today)));

% Output: the response pass (your existing naming, same date)
outDir  = fullfile('/Users/kate/Documents/retina-model', ...
                   [stamp '_runAllImages']);
...
% rootDir = '/Users/kate/Documents/retina-model/2025-11-13_pilot_patch'; % '/Users/kate/Documents/retina-model/image-set'; 
splits  = {'train','val','test','chips'}; 
imgList = {};
...
% end
function scene = sceneFromXYZfloat32(tiffPath, sceneFOVdeg)
% Minimal XYZ(D65) -> scene helper (display sRGB code values, spectral display).
...
% linear -> *display* sRGB code values (gamma-encoded), still float
if exist('lrgb2srgb','file') == 2
    rgb_disp = lrgb2srgb(rgb_lin);
else
    % Fallback sRGB EOTF (approx)
    a = 0.055;
...
% Build spectral scene using a calibrated display
d     = displayCreate('LCD-Apple');
